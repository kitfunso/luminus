...
export const embeddedCapacityRegisterSchema = z.object({
  ...
  operator: z
    .string()
    .describe('Distribution operator. Supported: "UKPN", "SPEN", "all".'),
  radius_km: z
    .number()
    .optional()
    .describe(
      "Search radius in km (default 25, max 50). Only applies to UKPN (SPEN has no coordinates).",
    ),
  limit: z
    .number()
    .optional()
...
function getSourceMetadata(operator: SupportedOperator): GisSourceMetadata {
  const metadataKey = operator === "UKPN" ? "ukpn-embedded-capacity" : "spen-embedded-capacity";

  return (
    GIS_SOURCES[metadataKey] ?? {
      id: metadataKey,
      name: operator === "UKPN" ? "UKPN ECR" : "SPEN ECR",
      provider: operator === "UKPN" ? "UK Power Networks" : "SP Energy Networks",
      licence: "Open Data",
      url:
        operator === "UKPN"
          ? "https://ukpowernetworks.opendatasoft.com/explore/dataset/ukpn-embedded-capacity-register/"
          : "https://spenergynetworks.opendatasoft.com/explore/dataset/embedded-capacity-register/",
      api_key_required: true,
      coverage: operator === "UKPN" ? "UKPN" : "SPEN",
      update_frequency: "Periodic",
      reliability: "medium" as const,
      caveats: [],
      attribution: operator === "UKPN" ? "UKPN" : "SPEN",
    }
  );
}
...
export async function getEmbeddedCapacityRegister(
  ...
  // Spatial matching for UKPN records, alphabetical for SPEN (no coordinates)
  const ukpnFiltered = filtered.filter((r) => r.operator === "UKPN");
  const spenFiltered = filtered.filter((r) => r.operator === "SPEN");

  const ukpnMatched = ukpnFiltered
    .map((record) => ({
      record,
  ...
  // Combine: UKPN first (by distance), then SPEN (alphabetical)
  const allMatched = [...ukpnMatched, ...spenMatched].slice(0, limit);
